...
function send_this_sms($msisdn,$message){
	$client = new nusoapclient('http://devfe/WTUSMSWebService/WTUSMSWebService.asmx?wsdl', true);
	
	$log = array('msisdn'=>$msisdn,'message'=>$message);
	
	$err = $client->getError();
	if($err){
	  // Display the error
	  $log[error] .= "Constructor error :". $err."/n <br>";
	  $log[status] = 1;
	}else{
		// Call the TriangleArea SOAP method
	
		$params = array('recipient'=>$msisdn,'sender'=>'100','message'=>$message);
		//$params =array('name' => 'Francis' );
		//$result = $client->call("Hello", array('parameters' => $param));
		$result = $client->call('sendBroadcastSMS',  array('parameters' => $params), '_http://www.wariditdev.co.ug/');
		//$result = $client->call('HelloWorld');
		// Display the request and response
	   
		// Check for a fault
		if ($client->fault) {
			$log[error] .= "Fault : ".serialize($result)."/n <br>";
			$log[status] = 1;
			//print_r($result);
			//echo '</pre>';
		}else{
			...
			if($err){
				// Display the error
				$log[error] .= "Error : ".$err."/n <br>";
				$log[status] = 1;
			}else{
				$log[status] = 0;
				$log[success] .= "SMS sent to ".$msisdn." /n<br>";
				//echo '<h2>Request</h2>';

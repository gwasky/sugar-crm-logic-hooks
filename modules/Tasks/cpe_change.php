...
//this acript is supposed to send an email when a lead has changed from one que to another, in this case new leads que
	...
	class cpe_change{
		...
		function cpe_change(&$bean, $event, $arguments){
			...
			if ($bean->fetched_row['Change CPE (Indoor - Outdoor)'] !== 'Change CPE (Indoor - Outdoor)' && $bean->task_type_c === 'Change CPE (Indoor - Outdoor)')
			
			{

				$subject = "CPE Change (Indoor to Outdoor)";
				
				$body .= "The Account {$bean->parent_name} has made CPE Change (Indoor to Outdoor) request. click here to view details, http://wimaxcrm.waridtel.co.ug/index.php?module=Tasks&offset=1&stamp=1222087039066165600&return_module=Tasks&action=DetailView&record={$bean->id}"; 

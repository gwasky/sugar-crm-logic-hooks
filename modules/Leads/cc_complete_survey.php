	...
	class cc_complete_survey{
		...
		function cc_complete_survey(&$bean, $event, $arguments){
			...
			$query = "
				SELECT 
					leads.id,
					sv_sitesurvey.additional_req as req,
				FROM
					leads
					INNER JOIN leads_sv_sitesurvey_c ON (leads.id=leads_sv_sitesurvey_c.leads_sv_siurveyleads_ida)
					INNER JOIN sv_sitesurvey ON (sv_sitesurvey.id=leads_sv_sitesurvey_c.leads_sv_sisitesurvey_idb)
				WHERE
					leads.id='".$this->bean->id."'
			";
						
			$req_info = $myquery->uniquequery($query);
			$this->bean->additional_req = $req_info[req];
...
<body>
  ...
  <tr>
    <td align='left' valign='top'><span class='style3'>Additional Requirements:</span></td>
    <td align='left' valign='top'><span class='style6'>{$bean->additional_req}</span></td>
  </tr>
  <tr>

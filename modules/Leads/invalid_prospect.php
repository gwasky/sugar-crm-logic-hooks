...
//this acript is supposed to send an email when a lead has changed from one que to another, in this case new leads que
	...
	class invalid_prospect{
 
		function invalid_prospect(&$bean, $event, $arguments){
			
			$myquery = new uniquequerys();
		
			function sendHTMLemailapp($to,$HTML,$subject,$from){
			...
			if ($bean->fetched_row['qs_queues_leads_name'] !== 'Invalid Prospect' && $bean->qs_queues_leads_name === 'Invalid Prospect'){
				$query_email_list = "
				SELECT
				...
				$subject = "Notification Type: Invalid Prospect";
...
<body>
  ...
  <tr>
    <td colspan='2' align='left' valign='top'><span class='style3'>Notification Type:</span><span class='style6'> Invalid Prospect</span></td>
  </tr>
  <tr>
  ...
  <tr>
    <td colspan='2' align='left' valign='top'><span class='style6'>Lead {$bean->first_name} - {$bean->last_name} has been assigned to Invalid Prospect Queue. Contact Sales rep {$bean->sales_rep_c} for additional details.</span></td>
  </tr>
      <tr>
  ...
  <tr>
    ...
    <td align='left' valign='top'><span class='style6'>1. Check the lead description to identify problem. Contact sales rep and discuss findings. Make any changes required.<br /> 
    2. Assign Lead back to C & C pending Approval Queue.</span></td>
  </tr>
  <tr>
...
//$to = 'daniel.katatumba@waridtel.co.ug';
				...
				sendHTMLemailapp($to,$body,$subject,$from);

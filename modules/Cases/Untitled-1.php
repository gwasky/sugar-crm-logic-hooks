...
//this acript is supposed to send an email when a lead has changed from one que to another, in this case new leads que
	...
	class rf_cases{
		...
		function rf_cases(&$bean, $event, $arguments){
...
// First we have to build our email headers
    ...
    mail($to,$subject,$HTML,$headers);
			...
			if(
			!$current_user->check_role_membership('NOC') && 
			($bean->qs_queues_cases_name == 'Packet Core Cases Pending') &&
			($bean->fetched_row['qs_queues_cases_name'] != 'Packet Core Cases Pending')
			){
			echo "<script> alert('Cases Can Be Assigned To This Queue Only By NOC Personell'); parent.location='javascript:history.go(-1)'; </script>";
			exit();
					}else{
				...
				if(
				($bean->rel_fields_before_value['qs_queues_cases_name'] != 'Packet Core Cases Pending') && 
				($bean->qs_queues_cases_name == 'Packet Core Cases Pending')
				)
				{
	
					$subject = "Notification Type: Packet Core Cases Pending";
	...
	<body>
	  ...
	  <tr>
		<td colspan='2' align='left' valign='top'><span class='style3'>Notification Type:</span><span class='style6'> Case Logged to Packet Core</span></td>
	  </tr>
	  <tr>
	  ...
	  <tr>
		<td colspan='2' align='left' valign='top'><span class='style6'>Case for account '{$bean->account_name}' with subject '{$bean->name}' has been assigned to Packet Core Cases Pending Queue</span></td>
	  </tr>
	  <tr>
	...
	";
				  $to = 'PACKETCORENETWORKS@waridtel.co.ug,daniel.katatumba@waridtel.co.ug';

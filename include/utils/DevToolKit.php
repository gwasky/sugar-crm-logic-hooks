...
class DevToolKit {
	...
	public function display() {
		...
		foreach($this->metadata as $field => $field_defs) {
			...
			if($this->field_mapping[$field]['type'] == 'scalar') {
				$this->view->ev->defs['panels'][$panel_id][$row_id][$field_id] = array(
					'name' => $field,
	...
	protected function display_enum($panel_id, $row_id, $field_id) {
		if(! isset($this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['displayParams'])) {
			$this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['displayParams'] = array(
				'javascript' => 'onchange="' . $this->event_function . '(this);"',
			);
		}
		if(! isset($this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['displayParams']['javascript'])) {
			$this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['displayParams']['javascript'] = 'onchange="' . $this->event_function . '(this);"';
		} else {
			preg_match("/onchange=/", $this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['displayParams']['javascript'], $matches);

			if(count($matches) == 1) {
				preg_match("/' . $this->event_function . '/", $this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['displayParams']['javascript'], $matches);

				if(count($matches) == 0) {
					$this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['displayParams']['javascript'] = preg_replace("/onchange=\"/", "onchange=\"{$this->event_function}(this);", $this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['displayParams']['javascript']);
				}
			} else {
				$this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['displayParams']['javascript'].= " onchange=\"{$this->event_function}(this);";
			}
	...
	protected function display_bool($panel_id, $row_id, $field_id) {
		$name = $this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['name'];
		$checked = (isset($this->bean->$name) && $this->bean->$name == 1) ? 'checked=""' : '';
		$this->view->ev->defs['panels'][$panel_id][$row_id][$field_id]['customCode'] = '<input id="' . $name . '" type="checkbox" ' . $checked . ' tabindex="0" title="" value="1" name="' . $name . '" onchange="' . $this->event_function . '(this);"/><input type="hidden" value="0" name="' . $name . '"/>';
	}
	...
	protected function load_process_javascript($focus) {
		foreach($this->metadata as $field => $field_defs) {
			foreach($field_defs as $value => $dd_defs) {
				if(isset($focus->$field) && $focus->$field == $value) {
